...
public class A {
    ...
    public static void main(String[] args) {
        ...
        while(t > 0) {
            ...
            for(int i = 0; i < n; i++) {
                out.print(segmentTree.calc(a[i],  segmentTree.calc.length-1).value+" ");
                segmentTree.set(a[i], 1);
            }
            out.println();
        }
        out.flush();
    ...
    static class SegmentTree {
        ...
        public ITEM find(int k, int l) {
            return find(k, 0, l, 0, size);
        }

        private ITEM find(int k, int x,int l, int lx, int rx) {
            if(calc[x].value < k) return new ITEM(-1);
            if(rx <= l) return new ITEM(-1);
            if(rx - lx == 1) return new ITEM(lx);
            int m = (lx + rx) / 2;
            ITEM res = find(k, 2 * x + 1, l, lx, m);
            if(res.value == -1) {
                return find(k, 2 * x + 2, l, m, rx);
            }
            return res;
        }
        ...
        private void set(int i, int v, int x, int lx, int rx) {
            if(rx - lx == 1) {
                calc[x] = new ITEM(calc[x].value + SINGLE_ELEMENT(v).value);
                return;
            }
            int m = (lx + rx) / 2;
            if(i < m) {
