...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        char [] input = (s+t).toCharArray();

        out.println(getLCP(input, lens, lent));
        out.close();
    }

    private static String getLCP(char[] input, int lens, int lent) {
        ...
        for(int i = 0; i < lcp.length-1; i++) {
            if((p[i] < lens  && p[i+1] >= lens) || (p[i] >= lens && p[i+1] < lens)) {
                int l_max = lcp[i+1];
                if(l_max > max) {
                    max = l_max;
